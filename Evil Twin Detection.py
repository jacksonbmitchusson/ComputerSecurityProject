...
def monitor_networks(target, exceptions, hamming_dist_threshold=5, char_equivs=[], delaytime=1):
    detected_ssids = []
    while(True):
        print('Searching...')
        ssids = [ssid for ssid in get_wifi_networks() if ssid not in detected_ssids and ssid not in exceptions]
...
exceptions = ['network_a', 'network_b']
target = 'target_network'

monitor_networks(target, exceptions)